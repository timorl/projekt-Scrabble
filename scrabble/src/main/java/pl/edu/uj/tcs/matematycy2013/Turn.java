...
public class Turn {
    ...
    private boolean check(int X, int Y, LinkedList<LetterCoordinates> coors) {
        ...
        length = checkLine(-X, -Y, beg.x, beg.y, length, coors);
        if (length >= board.getSize() || !coors.isEmpty()) {
            return false;
        }
        else {
            for (LetterCoordinates co : onBoard) {
                if (board.isEmpty() && checkStart(co)) {
                    return true;
                }
                else if (checkNeighbours(co)) {
                    return true;
                }
            }
            return false;
    ...
    private boolean checkStart(LetterCoordinates co) {
        if (board.getBoard()[co.x][co.y] == Cell.START) {
            return true;
        }
        return false;
    }
    ...
    private boolean isValid() {
        ...
        if (coors.size() == 1) {
            if (board.isEmpty()) {
                return checkStart(coors.getFirst());
            }
            ...
            if (c1.x == c2.x) {
                return check(0, 1, coors);
            }
            else if (c1.y == c2.y) {
                return check(1, 0, coors);
            }
            else {
                return false;
            }
    ...
    public ArrayList<LetterCoordinates> getOnBoard() {
        return onBoard;
    }
